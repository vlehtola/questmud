/* binds x,y coordinates on outmap to castle id */
/* locations are not to be removed! */

#define SAVEFILE	"/data/castle/CASTLE_LOCATION"
...
string get_castle_entrance(int x, int y) {
  ...
  for(i=0; i < sizeof(coords); i++) {
    if(coords[i][0] == x && coords[i][1] == y) {
      file = "/data/castle/castle_"+m_indices(locations)[i]+"/entry";
...
status valid_location(int x, int y) {
  ...
  /* unwanted castle locations. a location must also be a loadable vmap room */
  switch(char) {
    case "-": return 0;
    case "|": return 0;
    case "+": return 0;
...
#ifdef DEBUG
  if(this_player()->query_wiz())
    write("Getting castle entrace exit dir for castle_id "+castle_id+".\n");
#endif
