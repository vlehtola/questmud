...
string name, report, path,tmp;
        ...
        write("Listing known bugs: (Type 'bug all' to list all known bugs.)\n");
        ...
        if(name == "here") {
                ...
                BUG_D->add_bug(name,path,report,this_player()->query_real_name(), time() );
                return 1;
        }
        ...
        //Added query_wiz() -- Rag 11.5.2004
        if(name == "end" && this_player()->query_wiz() ) {
                write("Ending bug number "+report+"\n");
                i = to_int(report);
                BUG_D->remove_bug(i);
                return 1;
        }
