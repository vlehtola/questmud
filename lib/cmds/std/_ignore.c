...
// Called by player.c, return 1 if the "str" player is ignored
// by this player, returns 0 if not (or the "str" player is 
// not in the game -> wont recieve/send messages anyway)
// Note that this also checks for level ignore!
/********************/

query_ignore(str, thisman) {
string *ignore_list;
string *arr;
...
if(!ob || !thisman) return 0;

// Mortals can't ignore wizzes
if(ob->query_wiz() && !thisman->query_wiz()) return 0;

ignore_list=thisman->query_ignored();
friends_list=(mapping)thisman->query_friends();
...
for(i=0;i<=20;i++) {
if(ignore_list[i] && i) {
if(lower_case(ignore_list[i])==ob->query_real_name()) return 1;          
     
                        }                    
                   }

// Check if on friends list (bypasses level ignore but not specific)
while(x--) {  if(arr[x]==ob->query_real_name()) return 0; }
// Check level ignore
