...
cmd_who(string str) {
  ...
  string *options;
  string tmp1,tmp2;
  int a, size, i, numba, players, unidle_mortals;
  ...
  if(options) {
    if(member_array("wiz",options) != -1)
      user = filter(user,"filter_users",this_object(),WIZ_ONLY);
    else if(member_array("mortal",options) != -1)
      user = filter(user,"filter_users",this_object(),MORTAL_ONLY);
    else if(member_array("newbie",options) != -1)
      user = filter(user,"filter_users",this_object(),NEWBIE_ONLY);
    else if(member_array("midbie",options) != -1)
      user = filter(user,"filter_users",this_object(),MIDBIE_ONLY);
    else if(member_array("highbie",options) != -1)
      user = filter(user,"filter_users",this_object(),HIGHBIE_ONLY);
    else if(member_array("max",options) != -1 && sizeof(options) > 1)
      user = filter(user,"filter_users",this_object(),LEVEL_MAX_ONLY,options[(member_array("max",options)+1)]);
    else if(member_array("min",options) != -1 && sizeof(options) > 1)
      user = filter(user,"filter_users",this_object(),LEVEL_MIN_ONLY,options[(member_array("min",options)+1)]);
    else if(member_array("idle",options) != -1)
      user = filter(user,"filter_users",this_object(),IDLE_ONLY);
    else if(member_array("unidle",options) != -1)
      user = filter(user,"filter_users",this_object(),UN_IDLE_ONLY);
  }

  user = filter(user,"filter_users",this_object());
  ...
  if(options) {
    if(member_array("short",options) != -1) {
      result = "";
      numba = 0;
      resulti = "QuestMud - http://questmud.no-ip.org - telnet://questmud.no-ip.org:3000 \n";
 ...
 for(a=0;a<size;a++) 
 {
  ...
  result = "";
  resulti = " "+bold_color("Level")+"   |  "+bold_color("Name")+"\t\t\t"+bold_color("QuestMud")+" - questmud.no-ip.org 3000\n";
  resulti += HR;
  players = 0;
...
// added by C
 ...
 resulti += bold_color( ""+ players ) +" players, "+ bold_color( ""+ mortals ) + " mortals, " +
                bold_color( ""+ unidle_mortals ) + " unidle mortals.";
