...
string color_file(string str, int size, string path);
  ...
  if(verbose) // Verbose mode output
     ...
     for(i = 0;i < sizeof(result); i++)
	{
	 buffy += sprintf("%s%-30s%s %15s  %20s\n",
			color_file(result[i][0], result[i][1], path),	
			result[i][0],
			OFF,
...
string color_file(string str, int size, string path)
{
 ...
 // .c
 if(find_object(path[0..strlen(path)-2]+str)) return colorize(str, "light_green");
