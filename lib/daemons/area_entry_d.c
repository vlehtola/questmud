...
check_special_coords(int x, int y) {
  ...
  for(i=0;i<sizeof(entries);i++) {
    if(entries[i][0] == x && entries[i][1] == y)
        str = entries[i][2];
    if(str) return str;
  }
  if(x == 112 && y == 114) {
...
void add_road(int x, int y, string mark) {
  mapping new;
  if(!roads) roads = ([ ]);
  new = ([ m_sizeof(roads) : x ; y ; mark ]);
  roads += new;
  VMAP->set_special_mark(new[0,0], new[0,1], new[0,2]);
  save_object(SAVEFILE);
}

status remove_road(int x, int y) {
  int i;
  for(i=0; i< m_sizeof(roads); i++) {
    if(roads[i,0] == x && roads[i,1] == y) {
      roads -= ([ i ]);
      save_object(SAVEFILE);
      return 1;
    }
...
void reset_special_marks() {
 ...
 for(i=0; i < m_sizeof(roads); i++) {
    VMAP->set_special_mark(roads[i,0], roads[i,1], roads[i,2]);
  }
}
...
reset(mixed arg) {
  ...
  //({ 190, 44, "/wizards/kaappi/mine/entrance", "?", }),
  ({ 190, 155, "/wizards/kaappi/mine/entrance", "?", }),
  //({ 267, 118, "/wizards/moonstar/areas/brownie/rooms/entrance", "?", }),
  ({ 140,161, "/wizards/ahma/ice/rooms/101", "?", }),
  //({ 100,116, "/wizards/moonstar/areas/mf/room/entrance", "?", }),
  ...
  ({ 256, 164, "/wizards/yodin/areas/forest/ff1", "?", }),
  ({ 185, 69, "/wizards/siki/lizard/entrance", "?", }),
  ...
  ({ 277, 110, "/wizards/rag/ruins/entrance", "?", }),
  ...
  ({ 100, 118, "/wizards/yodin/areas/windmill/wm1", "?", }),
  ({ 96, 112, "/wizards/yodin/areas/village/vh1", "?", }),
...
/* #### MULTIEXIT areas after this line #### */
  ...
  /* City of Asgroth */
  //({ 281,122, "/wizards/moonstar/areas/asgroth/rooms/wbridge", "#", }),
  //({ 282,123, "/wizards/moonstar/areas/asgroth/rooms/nbridge", "#", }),
  //({ 283,122, "/wizards/moonstar/areas/asgroth/rooms/ebridge", "#", }),

  /* Kaapin city */
  ({ 196, 149, "/wizards/kaappi/dcity/rooms/tunnele", "#", }),
