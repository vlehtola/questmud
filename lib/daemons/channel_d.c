...
/* Implemented 'last <number>' and added 20 more lines to history.
 * Also made some other cosmetic changes. -N 1.2.2004
 */
// Added 'copy' channel for copy requests. -N 5. Feb 2004
...
static string *allowed_channels;
 ...
 reset(arg) 
 {
   ...
   allowed_channels = ({"inform","newbie","chat","sales","mud","ghost",
                        "fin","swe","code","wanted","alert","music","wiz","fiz","arch","adm","log","copy" });
  ...
  change_topic(chan,topic) {
    ...
    if(strlen(topic) > 40) {
      write("A maximum of 40 characters allowed.\n");
      return 1;
  }
  ...
  send_to_channel(chan,msg, object this_obj) 
   {
      ...
      if(msg) 
	{
         ...
         if(msg=="last") 
	   {
            write_channel_history(chan, 20);
            return;
           }

	 if( sscanf( msg, "last %d", lines ) == 1 )
	   {
            write_channel_history(chan, lines);
            return;
           }
...
write("Players below level "+LEVELLIMIT+" can only speak on the 'newbie' channel.\n");
      return; }   
       ...
       if(this_obj->query_wiz()) {
        ...
        if( sizeof(temp_hist) > MAX_LINES ) 
	{

          for( i=sizeof(temp_hist)-(MAX_LINES+1) ; i<sizeof(temp_hist) ; i++) 
	  {
        ...
        save_object(SAVEFILE);  // Our save_object is here. - Nalle

        global_channel=chan;
        who=filter(users(),"if_on_ch");
  ...
  int write_channel_who(string arg) {
    ...
    who=filter(who,"if_on_ch");
    who=filter(who,"is_visible");
...
channel_list() 
{
 ...
 if(!TP->query_terminal())
  write(sprintf("%-10s","Channel")+sprintf("%-10s","Status")+sprintf("%-20s","Description")+"\n");
  else
  write(BOLD+sprintf("%-10s","Name")+sprintf("%-10s","Status")+sprintf("%-20s","Description")+OFF+"\n");
 ...
 for(i=0;i<sizeof(temp);i++) 
 {
  if(valid_join(temp[i])) 
    {
     tmp_color = COLOURS->give_color(TP, temp[i]);

     str=sprintf(tmp_color+"%-10s"+OFF, " ["+temp[i]+"]")+"";
     temp2=this_player()->query_channel();

     if(member_array(temp[i],temp2)==-1) 
	{ if(TP->query_terminal()) str=str+"["+RED_F+"OFF"+OFF+"]"; else str=str+"[OFF]"; }
         else 
	{ if(TP->query_terminal()) str=str+"["+GREEN_F+"ON"+OFF+"] "; else str=str+"[ON] "; }
 ...
 write("A total of "+n+" channels displayed.\n");

 return 1;
}


write_channel_history(chan, int lines) 
{
 string *history;
 int i;

 if(!lines || lines < 1 || lines > MAX_LINES) lines = 20;

 history = channels[chan,3];

 if (lines >= sizeof(history)) lines = sizeof(history)-1;

 write("Last "+lines+" messages on channel "+chan+":\n");

 for(i = sizeof(history) - lines ; i<sizeof(history) ; i++) 
    {
      write(history[i]+"\n");
    }
