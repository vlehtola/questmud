// You don't want to know what this is. - Nalle, level 1000.
...
string last_message_date = "Not App";
...
status input(string str) // entinen wwb_cmd
{
  ...
  // Member list
  chan_members=({"nalle","walla","houtmi", "waltest"});
 ...
 // Datestamp the first message of the day
 if( last_message_date != ctime( time() )[4..9] )
   {
    str = "(" + ctime( time() )[4..9] + ") " + str;
    last_message_date = ctime( time() )[4..9];
   }
...
void fuckup_messagedate() { last_message_date = "Not App"; }
...
string *buffy;
  ...
  last_messages+=({ STEGANO->rot13      ( (string) 
        ( "[" + ctime()[11..15] + "] " + construct_chanstr(ob, str, 1) ) 
                                        ) });
