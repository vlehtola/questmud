...
get_weapon_skill(arg) {
  ...
  } else if(!arg->query_warlock_weapon() && arg->query_type() != "knuckles") {
  i = attacker->query_skills(arg->query_type()+"s") + attacker->query_skills("attack")/3;
  }

  if(arg->query_type() == "knuckles") {
   i = attacker->query_skills("martial arts") + attacker->query_skills("attack")/3;
  }
...
display_combat_messages(object at, object at_ob) {
  ...
  if(!attacker) return; // is this check necessary? ++Celtron
  if (at_ob && present(at_ob,environment(at))) {
    tell_room(environment(at), aname+" is in vigorous combat with "+dname+".\n", ({at,at_ob}));
  } else {
    return;
  }
  return; /* bugi pois */ // this feature should be put back into play with call_out ++Celtron
...
static get_monster_hits() {
  ...
  /* upgrade high level monster damage 20.05.2003 //Celtron */
  /* downgraded a bit 07.06.2003 */

  threshold = attacker->query_level() - 30;
  if(threshold < 1) return;

  threshold = 125+ threshold*3;
  if(threshold > 400) threshold = 400;
  for(i=0;i<4;i++) {
    number_of_hits[i] = number_of_hits[i]* threshold /100;    
  }
}
...
attack(int power,object ob,int hi) {
  ...
  if(!deceive_hit() && !attacker_ob->query_stun()) {
    if(!attacker_ob->query_defend(1)) {
      // berserking disables dodging but parry is still allowed
      if(!attacker_ob->query_berserk()) {
      ...
      parry = do_parry();
      if(parry == -1) {
	attacker_ob->remove_one_mirror_image(); // added by Celtron 200208
        return 0;
      }
    } else { /* skill "defend" succesful */
      return 0;
    }
  }
  if(parry) {
...
hit_chance(object ob) {
  ...
  if(name_of_weapon && name_of_weapon->query_type() == "knuckles")
  {
   sk = attacker->query_skills("martial arts")*2;
   hb = name_of_weapon->query_hit_bonus(attacker->query_str());
  }
...
/*    str = attacker_ob->query_left_weapon()->query_name(); */
  ...
  if(name_of_weapon && !random(3) && name_of_weapon->query_type() != "knuckles")
	name_of_weapon->check_weapon_break();
...
check_hit_through() {
  ...
  if(shield && !random(3) && shield->query_type() != "knuckles") {
...
parry_chance(object ob, int shield) {
  ...
  // for monsters, they do not have always weapons/shield
...
// artists cannot parry with knuckles. Celtron
    if(!attacker_ob->query_right_weapon() ||
	 attacker_ob->query_right_weapon()->query_type() == "knuckles")
	return 0;
...
calculate_damage(string type) {
  ...
  if(wt == "bludgeon") {
	...
	mastery = attacker->query_skills("advanced martial arts");
  }
  ...
  if(base_dam > 20) base_dam = 20 + (base_dam-20)*9/10;
   ...
   if(base_dam > 105000) base_dam = 105000;  //C 291008
