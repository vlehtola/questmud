...
if(find_player(array[i]) && 
   !find_player(array[i])->query_invisible()) target=array[i];  
 else
...
if(!present(target, environment(this_player())) 
	&& find_player(target) && !find_player(target)->query_invisible()) 
