/* guildrank_d for getting guild ranks and calculating the guild leader by
Celtron */
...
string *query_guilds() {
  ...
  guild =       ({
                ...
                "necromancer", "witch", "psionic", "warlock", "martial artist", "sorcerer",
                });
...
void get_leader_ob(string guild) {
  ...
  if(guild == "sorcerer") item = "sorcerer_cape";
...
get_rank_status(string guild, int i) 
{
  ...
  if(guild == "fighter")
    ranks = ({"a brawler", "a soldier", "a corporal", "a sergeant", "a lieutenant", "a commander", "a general","a Champion of Deleria",});
  if(guild == "cleric")
    ranks = ({"an initiate", "a cleric", "a preacher", "a priest", "a high priest", "a bishop", "a cardinal","a Voice of Cyral",});
  if(guild == "mage")
    ranks = ({"an apprentice", "a magician", "a chanter", "a mage", "an elementalist", "an adept", "an archmage","a Grandmaster of Essence",});
  if(guild == "abjurer")
    ranks = ({"an apprentice abjurer", "a spellweaver", "an enchanter", "an abjurer", "a protector", "an adept protector", "a master protector","a Sacred Shielder",});
  if(guild == "necromancer")
    ranks = ({"a child of decay", "a bringer of sorrow", "a lifebane", "a necromancer", "a soulsucker", "a tool of death", "a spawn of terror","a Lord of the Undead",});
  if(guild == "witch")
    ranks = ({"a sybil", "a hermit", "a hag", "a crone", "a witch", "a mystic", "a witch of the dark wood","a witch of the dark wood",});
  if(guild == "psionic")
    ranks = ({"a clairsentient", "a psychokinetic", "a psychoportive", "a telepathic", "a metapsionic", "a master psionic", "a supreme psionic","a Mindcrusher",});
  if(guild == "warlock")
    ranks = ({"a warlock", "a warlock", "a warlock", "a warlock", "a warlock", "a master warlock", "a grand warlock", "an Elder warlock",});
  if(guild == "martial artist")
    ranks = ({"a takitu", "a ryotaka", "a zasenro", "a wahringust", "a sabumnim","a sarinku", "a saizenso", "a Master of mind and body",});
  if(guild == "sorcerer")
   ranks = ({"an apprentice scholar of the will", "a scholar of the will", "an apprentice sorcerer of the will", "a sorcerer of the will",
   "an aura creator of the will","a grand sorcerer of the will", "an archsorcerer of the will","an archsorcerer of the will", });

  if(!max_scores[guild]) define_max_score(guild);
  max_sc = max_scores[guild];

  /* n% of max score. ranks [0,9](no rank) ; 10% 20% 30% 40% 50% 60% 70% (90% added by Rag 25.7.04)*/
  n = i*100 / max_sc;
  n = n / 10;
  if(!n) return 0;
  n -= 1;
...
get_rank_level(string guild, int i)
{
  int max_sc, n;
  string ranks;
  if(!i) return 0;
...
query_rank(mixed str, string guild, status verbose) {
  ...
  if(guild && is_member_of_guild(ob, guild)) {
    ...
    if ( get_rank_level(guild, rank_score ) == 7 ) {
      return best_phrase(ob, rank_status, guild[i]);
    }
  ...
  while(i < sizeof(guild)) {
    if(is_member_of_guild(ob, guild[i])) {
      ...
      rank_score = count_rank_score(ob, guild[i],0,verbose);
      
      if ( get_rank_level(guild[i], rank_score ) == 7 ) {
        rank_status += best_phrase(ob, get_rank_status(guild[i], rank_score ), guild[i] );
      } else {
        rank_status += make_phrase(ob, get_rank_status(guild[i], rank_score ) );
      }
    }
    i += 1;
  }
...
string best_phrase(object ob, string rank_status, string guild) {
  string c;
  if ( !rank_status ) return "";
  if ( this_player()->query_terminal() && guild ) {
    if ( guild == "fighter" ) c = BLUE_F;
    if ( guild == "cleric" ) c = CYAN_F;
    if ( guild == "mage" ) c = RED_F;
    if ( guild == "necromancer" ) c = WHITE_F;
    if ( guild == "abjurer" ) c = MAGENTA_F;
    if ( guild == "psionic" ) c = GREEN_F;
    if ( guild == "martial artist" ) c = YELLOW_F;
    rank_status = c + rank_status + OFF;
  }
  return capitalize(ob->query_pronoun())+" is "+rank_status+".\n";
}

define_max_score(string guild) 
{
  if(max_scores[guild]) return max_scores[guild];
  max_scores += ([ guild : count_rank_score(0, guild, 1) ]);
  return max_scores[guild];
}
...
string get_guild_dir(string guild) {
  switch(guild) {
    ...
    case "sorcerer": return "/guilds/sorcerer/";
  }
  return 0;
}

/* returns (int)rank for a whole guild */
count_rank_score(object player, string guild_name, status max_score, status verbose) {
  int rank_score,i;
  string files, dir;
  ...
  while(i < sizeof(skill_names)) {
    if(max_score) {
      rank_score += 100 * SKILLFUN->skill_cost(skill_names[i]);
         ...
         rank_score += (int)(player->query_skills(skill_names[i])) *
(int)(SKILLFUN->skill_cost(skill_names[i]));
    }
    i += 1;
  }
  return rank_score;
...
query_highest_rank(object player) {
  ...
  int i, cond_max, tmp, max_sc;
  guilds = query_guilds();
  for(i=0; i < sizeof(guilds); i++) {
    tmp = count_rank_score(player, guilds[i]); /* * 100 /
    (count_rank_score(player, guilds[i], 1)+1); */
    if(!max_scores[guilds[i]]) define_max_score(guilds[i]);
    max_sc = max_scores[guilds[i]];
    tmp = tmp*100 / max_sc;
    if(cond_max < tmp) {
      cond_max = tmp;
