...
query_rank(mixed str, string guild, status verbose) {
  ...
  while(i < sizeof(guild)) {
    if(is_member_of_guild(ob, guild[i])) {
      ...
      // Finger now only shows the rank if it is the highest rank. -N Jan '04
      if( file_name(previous_object()) != "/cmds/std/_finger" || get_rank_level(guild[i], count_rank_score(ob, guild[i],0,verbose)) == 6)
