...
mapping leader_scores, leader_names;

void reset(status arg) {
  if(arg) return;
  max_scores = ([ ]);
  leader_scores = ([ ]);
  leader_scores = ([ (string) "resetti" : (int) 1 ]);
  leader_names = ([ ]);
...
string *query_guilds() {
  string *guild;
  guild = ({"fighter", "cleric", "mage", "abjurer", "necromancer", "witch", "psionic", "warlock",});
  return guild;
}
...
string query_guild_leader(object player) {
  ...
  while(i<sizeof(guild)) {
    if(leader_names[guild[i]] == player->query_real_name())
...
reset_guild_leader(string guild, string player_name) {
  ...
  string guilds;
  if(guild) {
    leader_names[guild] = 0;
    leader_scores[guild] = 0;
    return 1;
  }
  if(!player_name) return 0;
  guilds = query_guilds();
  for(i=0; i < sizeof(guilds); i++) {
    if(leader_names[guilds[i]] == player_name) {
      leader_names[guilds[i]] = 0;
      leader_scores[guilds[i]] = 0;
    }
  }
  save_leaders();
}
...
try_lead() {
  ...
  for(i=0; i < sizeof(guild); i++) {
    ...
    if(leader_scores[guild[i]] < score && score > 2000 &&
       leader_names[guild[i]] != this_player()->query_real_name()) {
  ...
  // mark the new guildmaster
        leader_scores[guild[i]] = score;
        leader_names[guild[i]] = this_player()->query_real_name();
        write("Your supreme knowledge outranks the other guild members. All hail the new guildmaster!\n");
        get_leader_ob(guild[i]);
      }
  // restore the guildleader items after entering the game
    } else if(leader_names[guild[i]] == this_player()->query_real_name()) {
      get_leader_ob(guild[i]);
    }
  }
  save_leaders();
...
get_rank_status(string guild, int i) {
  ...
  if(guild == "abjurer")
    ranks = ({"an apprentice abjurer", "a spellweaver", "an enchanter", "an abjurer", "a protector", "an adept protector",
        "a master protector",});
  if(guild == "necromancer")
    ranks = ({"a child of decay", "a bringer of sorrow", "a lifebane", "a necromancer", "a soulsucker", "a tool of death",
        "a spawn of terror",});
  if(guild == "witch")
...
query_highest_rank(object player) {
  ...
  guilds = query_guilds();
  for(i=0; i < sizeof(guilds); i++) {
    tmp = count_rank_score(player, guilds[i]); /* * 100 / (count_rank_score(player, guilds[i], 1)+1); */
    if(cond_max < tmp) {
      cond_max = tmp;
