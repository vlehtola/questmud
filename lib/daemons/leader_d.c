...
if( (this_player()->query_wiz() && this_player()->query_race() != "coder") || this_player()->query_tester() || this_player()->query_guest()) {
...
if(this_player()->query_race() == ob->query_race()) {
 ...
 // Changed since time from 20 to 180 on 041003 -- Nalle
 if(this_player()->query_level() < level && since < DAYS) {
     write("You are not enough powerful to get the leadership.\n");
     return 1;
 }

 else if(this_player() -> query_level() == level && since < DAYS) {
