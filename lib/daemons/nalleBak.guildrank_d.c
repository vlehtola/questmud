/* guildrank_d for getting guild ranks and calculating the guild leader by
   Celtron */
// Changed finger output to come on one line. -Nalle 21.01.04
...
get_rank_status(string guild, int i) 
{
  ...
  if(guild == "martial artist")
    ranks = ({"a takitu", "a ryotaka", "a zasenro", "a wahringust", "a sabumnim","a sarinku", "a saizenso", });
...
query_rank(mixed str, string guild, status verbose) 
{
  ...
  while(i < sizeof(guild)) 
  {
    if(is_member_of_guild(ob, guild[i])) 
    {
     ...
     tmp = get_rank_status(guild[i], count_rank_score(ob, guild[i],0,verbose) );
 ...
 if( rank_status && strlen(rank_status) > 3 )
  {
   if(strstr(",", rank_status[0 .. strlen(rank_status)-3] ) != -1)
    result = capitalize(ob->query_pronoun())+" has the ranks of "+rank_status[0 .. strlen(rank_status)-3]+".\n";
     else
    result = capitalize(ob->query_pronoun())+" has the rank of "+rank_status[0 .. strlen(rank_status)-3]+".\n";
  }
