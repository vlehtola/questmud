...
/*
  Tämmöstä herjaa heitti. Pistin tonne yhen lisätsekin jota en testaillu
  vielä mut tais siitä johtua että jotenki primary guild kusas. -N

  03:01 [log]: Anna /log/ENTER|Anna, Dec 30 03:01, 66.65.229.83 (19)
  03:02 [log]: Anna /log/BUG_ERROR|Dec 30 03:02 [anna] (env:/guilds/fighter/
    blades) [cmd:train slash to low] #(value)Indexing on illegal type.
 */
...
starter_guild_advance(string guild, string *starter, object player_ob) {
  ...
  if(player_ob->query_guild_level(guild) > BACKGROUND_MAX_LEVEL) {
	return;
  }
...
advance(string arg, int joining) {
    int level_cost, next_level, quest_cost, free_value;
...
train(skill) {
    ...
    if(!starter_guild) // -Nalle 311203
    { 
     write("No starter guild, inform an archwizard.\n");
     return 1;
    }
