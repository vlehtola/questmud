...
reset(arg) {
    ...
    set_level(30);
    set_name("guard");
    set_short("A guard dressed in deep blue and white clothes");
    ...
    weapon = clone_object("/world/eq/weapons/halberd.c");
...
init() {
    ::init();
    if(this_player()->query_guild_level("apprentice mages") &&
        !this_player()->query_quest("mage_test_failed")) {
        return 1;
    }
    if (this_player()->query_guild_level("abjurer")) {
        return 1;
    }
...
enter() {
    ...
    write("Guard blocks your way and grunts: 'Only mages and abjurers are "+
        "allowed to enter.\n");
    return 1;
}
