...
reset(arg) {
  ...
  long_desc =
"The guild room of the Unholy Lords. Peaces of different kinds of bones lie\n"+
...
check_joining_rec() {
  if(this_player()->query_guild_level("adept controllers") < 8) {
    write("You must first complete the guild of Adept controllers.\n");
    return 0;
  }
  if(this_player()->query_guild_level("master animators") < 7) {
    write("You much first complete the guild of Master animators.\n");
    return 0;
  }
  if(this_player()->query_guild_level("necromancer adepts") < 8) {
    write("You much first complete the guild of Necromancer adepts.\n");
    return 0;
  }
  if(this_player()->query_guild_level("school of dark power") < 5) {
