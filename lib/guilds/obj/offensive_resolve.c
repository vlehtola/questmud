...
offensive_resolve(type,class,category,effect_bonus,target,object caster) 
{
 ...
 string target_message, caster_message, say_message;
  ...
  switch(type) 
    ...
    reflect -= call_other( MASTERY_D , "query_mastery", "Offensive spellcasting" , caster)/10;
   ...
   damage += damage * call_other( MASTERY_D , "mastery", "Dangerous "+text_word , caster, ob) / 400;
   damage += damage * call_other( MASTERY_D , "mastery", "Offensive spellcasting", caster, ob) / 400;

//   damage += MASTERY_D->mastery( temp_str, caster, ob);
//   damage += find_object(MASTERY_D)->mastery( "Dangerous "+text_word , caster, ob);
