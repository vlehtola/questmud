...
start_spell(str, object caster) {
  ...
  if (sscanf(str, "%s %s %s at %s", category, class, type, target) == 4 ||
    ...
    if (type == "blt" || type == "strm" || type == "rhj" && !random(5)) {
      ...
      if(!target && caster_ob->query_attacker()) {
        if(environment(caster_ob) == environment(caster_ob->query_attacker()) ) {
          target = caster_ob->query_attacker()->query_name();
        }
      }
...
next_phase(status no_counter) {
    ...
    extra_time = get_word_text(no_counter);
    if (extra_time) {
        if (phase >= 3) {
            spell_effect();
            interrupt_spell(1);
...
get_time() {
	...
	control += MASTERY_D->query_mastery("Offensive spellcasting",caster_ob)/4;
        ...
        control += MASTERY_D->query_mastery("Knowledge of shielding",caster_ob)/4;
    } else if(category == "dcs") {
		control = caster_ob->query_skills("essence of chaos");
	}
...
/* if caster is dead by uncon*/
  ...
  if (phase == 3) {
    ...
    text_to_others = call_other(SPELLFUN, "get_text_to_others",word, caster_ob);

    tell_object(caster_ob, text);
    if(!elusive_check()) {
      tell_room(environment(caster_ob), text_to_others, ({ caster_ob }));
      notice_check();
    }
...
check_fail(word,text) {
  ...
  if(chance) {
    ...
    tell_object(caster_ob, "Syllable chance: "+chance_str+"%\n");
  }
...
cast_failure(int chance, object ob) {
  ...
  if(i == 2 && !( caster_ob->query_ability("stabilemagic") && random(10) ) ) {
...
counter_spell() {
  ...
  roll += MASTERY_D->query_mastery("Offensive spellcasting",caster_ob)/3;
  roll = random(roll+40);
...
spell_effect() {
    ...
    if (category == "chl") {
        if (!call_other("/guilds/obj/cleric_fail_check", "check",type,class, caster_ob)) {
           interrupt_spell(1);
           return 1;
        }
...
return 1;
    ...
    if (file_size("/guilds/spells/essence/_"+category+"_"+class+"_"+type+".c") != -1) {
        call_other("/guilds/spells/essence/_"+category+"_"+class+"_"+type,"resolve",effect_bonus, target, caster_ob);
        return 1;
    }
    if (file_size("/guilds/spells/channel/_"+category+"_"+class+"_"+type+".c") != -1) {
        call_other("/guilds/spells/channel/_"+category+"_"+class+"_"+type,"resolve",effect_bonus, target, caster_ob);
        return 1;
    }
    if (file_size("/guilds/spells/chaos/_"+category+"_"+class+"_"+type+".c") != -1) {
		call_other("/guilds/spells/chaos/_"+category+"_"+class+"_"+type,"resolve",effect_bonus, target, caster_ob);
        return 1;
	}
    if(category == "kyo") {
