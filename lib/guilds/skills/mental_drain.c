...
resolve_skill(str) {
    ...
    if(target->query_level() > MD_LVL_LIMIT + (MASTERY_D->query_mastery("Expanded mind",this_player())+10)/20) {
        ...
        att += MASTERY_D->mastery("Power of the mind",this_player(),target);
