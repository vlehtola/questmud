...
#define MAX_DAMAGE      2800  // max damage set to 2800 by Celtron 15102005.
#include <ansi.h>
...
dama = dama + this_player()->query_dex()/2;

// Your knuckle level actually affects the damage! (Celtron 18102005)
dama = dama * ob->query_right_weapon()->query_class() / 6;
...
if(dama > MAX_DAMAGE) dama = MAX_DAMAGE;
  ...
  case 2101..2590:
    ...
    tell_object(ob, "Huge beam of fire flies from your knuckles straight to "+target->query_name()+"'s face with devastating FORCE\n");
    tell_object(ob, "making "+target->query_name()+" scream in PAIN!\n");
    ...
    break;
  case 2591..3500:
    tell_object(ob, "You close your eyes and focus your mind and body for ultimate strike while screaming "+RED_F+"'RENZOKUKEN'"+OFF+"\n");
    tell_object(ob, "A blinding red light comes from your knuckles and hits "+target->query_name()+" while you jump high in the air.\n");
    tell_object(ob, "Flying through the air you STRIKE "+target->query_name()+"'s face and TEAR his body with FORCE!\n");
    tell_object(target, ob->query_name()+" flies towards you and STRIKE your face and RIPS your skin off with FORCE!\n");
    tell_room(environment(ob), ob->query_name()+" focuses all his powers and screams "+RED_F+"'RENZOKUKEN'"+OFF+".\n", ({ ob, target}));
    tell_room(environment(ob)," "+target->query_name()+" screams in pain as "+ob->query_name()+" STRIKES and RIPS off his skin with FORCE!\n", ({ ob, target}));
    break;
    }
