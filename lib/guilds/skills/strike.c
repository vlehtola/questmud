...
resolve_skill(str) {
    ...
    weapon_skill += this_player()->query_skills("strike")*2/3;
    ...
    if(dama > target->query_hp()*(100-target->query_resists(0))/100+2 && target->query_npc() && !target->query_demon()) {
    ...
    if(target) {
       if(this_player()->query_skills("double strike") > random(110)) {
            ...
            dama2 = dama * this_player()->query_skills("double strike") / 200;
