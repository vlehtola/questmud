...
start() 
{
 ...
 set_long("Sorcerers blue staff. kirjota pitempi desci. mainitse runeista\n"+
          "staffin corpse ala rank:"+get_short( staff_points )+"\n");

 set_stats("wis", (this_player()->query_skills("faith of izyldran")/10));
 set_stats("int", (this_player()->query_skills("mind link")/10));
 set_stats("spr", (this_player()->query_skills("knowledge of power")/10));

 // Aina kun tama esine clonetaan, haetaan vanha data tiedostosta:
 restore_object( SAVE_DIR+wielded_by->query_real_name() );
