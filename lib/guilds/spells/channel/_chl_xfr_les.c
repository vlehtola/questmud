...
// Cannot transfer hp to yourself
  if (ob == caster_ob) {
    tell_object(caster_ob,"You cannot transfer health to yourself.\n");
    return 1;
  }

//fixed by Celtron || Addon by Gynter (effect = )
  effect = "/guilds/spells/channel/_chl_hea_min"->get_heal_amount(bonus,caster_ob,ob);
...
/* Transferring ja healing ero: */
  ...
  if (ob == caster_ob || caster_ob->query_hp() == 1) {
    return 1;
  }
  ...
  " by transferring "+ caster_ob->query_possessive() +" own health.\n",({ ob }));
  if (ob != caster_ob) {
