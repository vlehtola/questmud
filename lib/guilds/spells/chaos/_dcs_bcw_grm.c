...
if(!target) {
tell_object(caster_ob, "Usage: cast dcs bcw grm at boost <amount>\n");
return 1;
}
...
tell_object(caster_ob, "You call blessing upon the chaos axe.\n");
tell_room(environment(caster_ob), caster_ob->query_name()+" calls blessing upon the chaos axe.\n", ({ caster_ob }));
