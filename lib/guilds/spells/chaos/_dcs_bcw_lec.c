...
if(!target) {
tell_object(caster_ob, "Usage: cast dcs bcw lecc at boost <amount>\n");
return 1;
}
...
tell_object(caster_ob, "You call blessing upon the chaos bludgeon.\n");
tell_room(environment(caster_ob), caster_ob->query_name()+" calls blessing upon the chaos bludgeon.\n", ({ caster_ob }));
