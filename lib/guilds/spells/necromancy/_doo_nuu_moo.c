...
if(!obs || sizeof(obs) < 1) {
  ...
  if(horde->query_statboost()) {
        if(horde->query_hpboost())write("You horde is already surrounded by dark red aura.\n");
        else write("Your horde is already surrounded by bright brown aura.\n");
        return 1;
        }

aura = "bright brown";
if(horde->query_hpboost())aura = "dark red"; 
...
i+= this_player()->query_skills("cast unholy enchantments");
 ...
 i+= MASTERY_D->mastery("Mastery of dark arts",this_player());
 
 i*=2;
 i/=3;
 
 horde->start_statboost(i);
