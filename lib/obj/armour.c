...
static int feather_weight_count;
...
short(int disable_colour) 
{
  ...
  if(this_player() && this_player()->query_npc()) 
	disable_colour = 1; // Quest-fix hack -N

  tmp = color;
  if(!tmp) tmp = "";
  else {
    if(!disable_colour && this_player() && this_player()->query_terminal() && color)
      tmp = color_for_short(color);
    tmp = " "+tmp;
  }

  if(!disable_colour)
    return ENVIRONMENT_D->give_color(this_player(), "armours")+short_desc+OFF+tmp+keep;
   else
    return short_desc+tmp+keep;
}

target_short(object target_player) 
{
  string keep, tmp;
  keep = "";
  if(environment(this_object()) && environment(this_object())->query_keep(this_object()))
  ...
  if(!tmp) tmp = "";
    ...
    if(target_player->query_terminal() && color)
      tmp = color_for_short(color);
    tmp = " "+tmp;
  }
  return ENVIRONMENT_D->give_color(target_player, "armours")+short_desc+OFF+tmp+keep;
}
...
// LEVEL LIMIT added by Celtron 18.01.2004
// Made a bit more playerfriendly by Nalle 20.02.2004
// Made a bit more playerfriendly by Nalle 14.05.2004 (was 4 now 6)

	if(ob[i]->query_ac()/6 > this_player()->query_level()) {
		write("You can't wear this intricate armour properly yet. (Level limit "+(ob[i]->query_ac()/4)+")\n");
		return 1;
	}
    	ob[i]->do_wear();
	done = 1;
...
stop_wearing(silently) {
 ...
 if (worn) {
   ...
   if(tmp_worn_by) tmp_worn_by->update_stats();
 }
}
...
remove(str,arg) {
  ...
  //Fix by Rag 30.6.2004
  if ( worn && (!worn_by || worn_by != this_player()) ) {
  	worn = 0;
  	worn_by = 0;
 	} // End of Ragfix
...
add_feather_weight_count() {
  feather_weight_count += 1;
  return feather_weight_count;
}

remove_feather_weight_count() {
  return feather_weight_count=1;
}

query_weight() { 
  int w;
  w = to_int(weight) / (feather_weight_count+1);
  return w;
 }
