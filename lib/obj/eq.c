/*
 * This is a new combined armour and weapon code file.
 * for compability reasons weapon.c and armour.c will be saved
 * this file adds multislot support and unique eq saving system
 * THIS FILE IS SELFSUPPORTED. OTHER BASE FILES ARE FOR OLD CODE COMPABILITY
 *
 *    //Celtron
 */
...
#define A_SUBTYPES "/daemons/armour_sub_types"
#define W_SUBTYPES "/daemons/weapon_sub_types"

string *slot, sub_type;
int quality, weapon;
int durability, max_durability;

string additional_desc, long_desc, short_desc, name;
int weight, value;

string wiz_flag;

mapping stats;		// statbonuses
string *resists; 	// ({ pref, amount%, }) one pref per item
mapping skill_bonus;

static object worn_by;
static int class;	// armour or weapon
static string color;

query_name() { if (!sub_type) { return name; } return sub_type; }
query_worn() { return worn_by; }
query_value() { return value; }
query_armour() { return !weapon; }
