...
look(string str,int brief, tp) {
  object ob, ob_tmp, obje,livingit;
  string item,lista,prefix;
  int max, is_light, i,ii;
  if (this_player()->query_uncon()) {
    write("You are unconcious and unable to see anything!\n");
    return 1;
  }
  ...
  if(str) return look_at(str,brief,tp, is_light);
  ...
  obje = all_inventory(environment(tp));
  livingit = allocate(sizeof(obje));
  i=0; ii=0;
  while(i<sizeof(obje)) {
    if(living(obje[i])) {
      livingit[ii] = obje[i];
  ...
  /* stackataan ekut */
  lista = call_other(tp, "stack_eq", obje);
  call_other(tp, "write_stack", lista);
  i=0; ii=0;
  /* displayataan livingit */
  prefix = "";
  if (is_light == 2) { prefix = "A dark figure of "; }
  while(i<sizeof(livingit) && livingit[i]) {
    if(livingit[i] != tp && livingit[i]->short()) {
      if (is_light == 1) { write("Something living in the darkness.\n"); }
      else { tell_object(tp,prefix + livingit[i]->short()+"\n"); }
    }
    i += 1;
  }
  return 1;
...
look_at(string str,int brief,tp, int is_light) {
  object ob, ob_tmp, obje,livingit, kohde;
  string item,lista,prefix;
  int max, i,ii;
  prefix = "";
