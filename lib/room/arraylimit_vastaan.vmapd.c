...
// Once the room-mapping contains 3700 rooms, cleanup is called
#define CLEANUP_ROOM_THRESHOLD 3700
 ...
 * (rooms mapping). see the defines above. -Nalle 25092004
 */
...
object get_room(string dir, string dest, int x, int y) 
{
  ...
  if( sizeof(m_indices(rooms)) > CLEANUP_ROOM_THRESHOLD )
    {
     clean_up_rooms();
    }
...
/* Experimental cleanup code. -Nalle  */
...
int clean_up_rooms()
{
 ...
 for(int i=0; i < sizeof(room_indices); i++)
    {
     object room_temp = rooms[ room_indices[i] ];

     if( time() - room_temp->query_last_entry() > CLEANUP_ROOM_THRESHOLD )
       {
	cleaned++;
	if( room_indices[i] != "load" ) rooms -= ([ rooms[ room_indices[i] ] ]);
       }
    }

 log_file("virtual_room_cleanup", ctime(time())+" Virtual room cleanup called ("+cleaned+" rooms destroyed).\n");
 return 1;
}
