...
query_short(int disable_colour) 
{
 string tmp;
...
int query_hash()
{
 string hashbuffer, result;
 hashbuffer = "                    ";

 // No hashes for outworld rooms
 if( query_virtual() ) 
 {
  return -1;
 }

 hashbuffer += file_name( this_object() );

 result = hashbuffer[strlen(hashbuffer)-9 .. strlen(hashbuffer)-1];
 result = hashbuffer[strlen(hashbuffer)-9 .. strlen(hashbuffer)-9]
