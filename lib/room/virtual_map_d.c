...
// Rooms are destroyed during cleanup if unvisited for 2h
#define CLEANUP_TIME_THRESHOLD 7200
// Once the room-mapping contains 2900 rooms, cleanup is called
#define CLEANUP_ROOM_THRESHOLD 2900

/* final version of virtual map daemon //Celtron */
/* optimizations to the map colouring code by Nalle 210404 */

/* virtual room cleanup system implemented to avoid mapping overflows
 * (rooms mapping). see the defines above. we need to change the cleanup
 * code to use filter_mapping, for speed and increased cache size.
 * cleanups are logged to '/log/virtual_room_cleanup'
 * -Nalle 28.09.2004
 */
...
int cleaned_up = 0;		// Only clean up once
...
void move_living(object walker, string arg, int x, int y) {
  ...
  if(x && y) 
    ...
    if(ep_cost >= 1) {
	ep_cost = ep_cost - 1 + random(2);
        ep_cost -= random(hiking)/18 + 1;
        if(ep_cost < 0) ep_cost = 1;
    }
  ...
  //environment() gave some funny bugs so I added a check for party_leader -- Rag 8.10.04
  if((!p_ob || this_player() == p_ob->query_leader()) ||
      (p_ob->query_leader() && environment(p_ob->query_leader()) != room) ) 
    {
...
status generate_room(int x, int y, string msymbol) {
  ...
  // Room exists in cache, no need to generate, just renew timestamp
  if(rooms[x+"_"+y])
    {
     rooms[x+"_"+y, 1] = time();
     return 1;
  ...
  /* Experimental code for cleaning up rooms. -Nalle */

  if( sizeof( rooms ) > CLEANUP_ROOM_THRESHOLD && !cleaned_up )
    {
     clean_up_rooms();
     cleaned_up = 1;
    }
...
/* Experimental cleanup code. Would be faster with a filter_mapping. -Nalle */
/* Remember the restrictions of MAX_ARRAY_SIZE until using filter_mapping.  */
...
int clean_up_rooms()
{
 ...
 room_indices = m_indices( rooms );

 for(int i=0; i < sizeof(room_indices); i++)
    {
     if( time() - rooms[ room_indices[i], 1 ] > CLEANUP_TIME_THRESHOLD )
       {
	if( room_indices[i] != "load" )
	   {
	    cleaned++;
	    destruct(rooms[ room_indices[i] ]);
	    rooms -= ([ room_indices[i] ]);
	   }
       }
    }

 log_file("virtual_room_cleanup", ctime(time())+" Virtual room cleanup called. "+cleaned+" rooms destroyed, "+sizeof(rooms)+" left.\n");
 return 1;
}

// For debugging
mapping query_rooms() 		{ return rooms; 	}
int	query_roomcount() 	{ return sizeof(rooms); }
