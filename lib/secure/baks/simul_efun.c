...
mixed valid_write(string str, object caller) {
  ...
  if(caller->query_real_name() == "moonstar" && (str[0..7] == "/guilds/" || str[0..19] == "/wizards/gynter/dark")) return str;
  if(caller->query_real_name() == "blind" && (str[0..14] == "/guilds/artist/" || str[0..15] == "/wizards/aarrgh/")) return str;
  if(caller->query_real_name() == "veron" && str[0..14] == "/guilds/artist/") return str;
  if(caller->query_real_name() == "archsin") return str;
