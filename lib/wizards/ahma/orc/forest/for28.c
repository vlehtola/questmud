...
reset(arg) {
  add_exit("west", "/wizards/ahma/orc/forest/for27");
  add_exit("southeast", "/wizards/ahma/orc/forest/for29");
  ...
  if(!alive) {
    ...
    if(h==5) { move_object(mob = clone_object("/wizards/ahma/orc/mob/wolf01"), this_object()); }
  }
  }
}
