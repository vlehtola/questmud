...
reset(arg) {
  ...
  if(arg) return;
  if (!monster) {
      monster = clone_object("/guilds/psi/celuwien");
      move_object(monster, this_object());
}

  set_short("The Psionicist guild");
  set_long("The Psionicists guild is the place where all people who start to\n"+
           "develop and improve their mind powers come. The guild room itself\n"+
           "is a simple and quiet place. A couple of chandeliers full of candles\n"+
           "are burning in both side of the guild room.\n");
  set_not_out();
  set_light(3);

  add_exit("west","/guilds/psi/fighter_psionicists");
  add_exit("east","/guilds/psi/caster_psionicists");
...
skill_list(int guild_level) {
  ...
  mapping skills;
	...
	if (guild_level >= 3) {
		...
		skills += (["stunned maneuvers":(int) 5]);
	...
	if (guild_level >= 7) {
		...
		skills += (["stunned maneuvers":(int) 20]);
	...
	if (guild_level >= 8) {
		...
		skills += (["stunned maneuvers":(int) 25]);
	...
	if (guild_level >= 11) {
		...
		skills += (["stunned maneuvers":(int) 35]);
	...
	if (guild_level >= 15) {
		...
		skills += (["stunned maneuvers":(int) 40]);
	...
	if (guild_level >= 17) {
		...
		skills += (["stunned maneuvers":(int) 45]);
