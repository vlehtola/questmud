...
start() {
  ...
  set_name("keeper");
  set_short("A prudent storagekeeper");
  set_long("A dirty looking storagekeeper covered with white flour. \n"
           "He is quite tall and muscular and seems to be wearing brown\n"
           "robe made from basic fabric and light brown apron made from\n"
           "leather. Storagekeeper looks very busy. He looks very happy.\n"
           "and seems to enjoy his work here.\n");
  set_level(100);
  set_gender(1);
  set_race("human");
  set_mage(1);
...
catch_tell(str) {
	...
	return 1;  				// exit and stop resolving the command
      ...
      tell_object(ob, "Keeper says: Thank you very much now we can make more bread.'\n");
