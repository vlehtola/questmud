...
int *num_rooms, id, i;
string room, castle, castledir, *file;
mixed *kind, *exits;
mapping owner;
...
int go_create(int type) {
  ...
  for(c=0; c<num_rooms[type]; c++) {
    generate_room(c, (string)kind[type][c], (string)exits[type][c]);
  }
...
generate_room(int room_num, string kin, string exts) {
  generate_room_init(kin, room_num);
  generate_room_desc(kin, room_num);
  generate_room_exit(room_num, exts);
  generate_room_finish(room_num);
  return 1;
}

int generate_room_init(string typ, int room_num) {
  string dira, dirb, code;
...
int generate_room_exit(int room_num, string xits) {
