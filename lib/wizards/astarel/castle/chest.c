...
set_combo(arg) {
  ...
  */
  if(!open && sealed) {
  	write("Chest must be open and unsealed!\n");
  return 1;
}
...
seal(arg) {
  ...
  if(!combo) {
  	write("You have to set the code first, DUMBASS!\n");
  	return 1;
}
  ...
  // Lets define new, free id.
  //id = 0;
  count = 0;
  this_room = file_name(environment(this_object()));
  sscanf(this_room, "/data/castle/castle_%d/room%d",number,tmp);
  tell_object(find_player("astarel"),"DEBUG :: number:"+number+" count:"+count+" ["+this_room+"]\n");
  while(file_size("/data/castle/castle_"+number+"/chest_"+count+".o") != -1) {
  	tell_object(find_player("astarel"),"DEBUG: number:"+number+" count:"+count+"\n");
    count += 1;
  }
  tell_object(find_player("astarel"),"DEBUG -- "+count+"\n");
  // and yet, lets add 1 more.
  if(count != 0) id = count;
...
de_seal(arg) {
  ...
  this_room = file_name(environment(this_object()));
  sscanf(this_room, "/data/castle/castle_%d/room%d",number,tmp);
  rm("/data/castle/castle_"+number+"/chest_"+id);
  environment(this_object())->create_chest_file();
  environment(this_object())->save_room();
  return 1;
}
...
close(arg) {
  ...
  open = 0;
  locked = 1;
  last_user_name = this_player()->query_name();
  current = random(10000)+1;
...
open(arg) {
  ...
  if(this_player()->query_name() == last_user_name) {
...
info(arg) {
  ...
  if(this_player()->query_wiz()) {
    ...
    write("Chest's last-user: "+last_user_name+"\n");
  }
  write(short()+"\n");
...
string query_last_user_name() { return last_user_name; }
set_last_user_name(string str) { last_user_name = str; }
