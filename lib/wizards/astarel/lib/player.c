...
update_stats() {
  ...
  if(query_rebirth() > 1 && race_exp_rate < 80)
...
write("+-----------------------+--------------------+-----------+-----------+\n");
	...
	/* Commented by Nalle Sept 03, replaced by an sprintf (below)
	    ...
	    if (strlen(("|"+skill_train[i])) < 26) { str = s[i], skill_costs[i],19)+"|"+str7+"|"+ str3+"|\n");
	  if (tester)
...
query_clan() {
  returt && set_short != "") { return set_short; }
...
link_death(mixed arg, status force) {
  ...
  time_ld = time();
  save_me(1);
  ob = present("party object",thisp_name+" goes link dead.");
  return 1;
}
	  ...
	  if (time_to_heal <= 0 && level != 0) {
		...
		query_thirst()*6 >= query_max_thirst()) {
		  ...
		  if (hit_point < max_hp) hit_point += hp_regen;
			...
			spell_points += sp_regen + sp_regen*STAT_D->mental_unity_percent(this_object(), sp_regen)/200;
...
/* Saman mokon tappoexpa reduce disable, koska liian vahan areoita + halutaa mortteja.
  ...
  if(sh) {
    ...
    if(query_kills(sh) > 100) {
      tell_object(this_object(), "You don't learn much new by killing the satmp = 80;
...
advance_(skill) skill->interrupt_skill();
}
