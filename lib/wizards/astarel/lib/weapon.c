...
#define BASE_WEAPON_WEIGHT 50  /* 0,5kg */
...
check_weapon_break() {
  if(query_type() == "knuckles") return 1;
  dur -= 1;
  if(dur < 0) {
...
short(int disable_colour) 
{
  ...
  if(this_player()->query_npc()) disable_colour = 1; // Quest-fix hack -N

  if(!disable_colour && this_player() && this_player()->query_terminal())
   st = ENVIRONMENT_D->give_color(this_player(), "weapons")+ short_desc + OFF + shape;
  else
   st = short_desc + shape;
...
target_short(object target_player) {
  ...
  if(target_player->query_terminal())
   st = ENVIRONMENT_D->give_color(target_player, "weapons")+ short_desc + OFF + shape;
  else
   st = short_desc + shape;
...
long(str) {
  write(long_desc);
  if(slayer) write("A rune is carved90+class) + (hb*20);
  /* weight = [100,800]? */
...
/* returns 0 if cannot, 1 for 1h, and 2 for 2h */
/* str = [20,250] size = [25,100] */
can_wield(int str, int size) {
  if(query_type() == "knuckles") { return 1; }
  str = str + size;
  if(query_forced2h() == 1)return 2;
...
weapon_names(num) {
    ...
    if (num == 5) { return "knuckles"; }
    return "Illegal weapon type (range [1,5])";
}
weapon_types(str) {
