...
#define MAX_LEVEL	320		// Up by 20 -N 12.2004
...
#define MORTAL_MAX_LEVEL	420	// Up by 20 -N 12.2004
...
status filter_real_players(object ob) {
	if(ob->query_level() < 25) return 0;
...
//  real_players = filter_array(users(), "query_level", this_object());
// No guests, logons, wizards and players under level 20 counted. (so 10 morts and EXP is ON!) - Asta 06.01.05
  real_players = filter_array(users(), "filter_real_players");
  
  if(sizeof(real_players) >= 10 && sh) {
