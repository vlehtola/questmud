...
#define PSI_IMEE_DEFULTA query_sp()
#define PSI_IMEE_DEFULTA_MAX query_max_sp()
#define PSI_IMEE_DEFULTA_REDUCE reduce_sp(dam)
...
int calc_dam() {
	...
	dam += att_hit*2;
	dam += att_dam*2;
	dam += this_player()->query_skills("mental strike")/2;
...
int get_psi_cost() {
	...
	temp = this_player()->query_level()/10; // 5
	...
	temp+= this_player()->query_int()/10; // 5 ?
	// 25
...
int dam,temp;
	...
	if(d->query_level() > TARGET_LVL_LIMIT) {
	//	tell_object(attacker,def_name+"'s mind is too powerful and beyond your powers!\n");
		return;
	}
	...
	if(defender->PSI_IMEE_DEFULTA < 1 && defender->query_npc()) {
		...
		if(!random(temp)) {
			...
			defender->set_sp(defender->PSI_IMEE_DEFULTA_MAX/2);
		} 
