...
reset(arg) {
  ...
  set_long("The apprentice Psionicists-guild.\n");
  set_light(3);
...
init() {
  ::init();
  add_action("psihelp","psihelp");
  add_action("psiitem","psiitem");
  guild_init();
}
...
status psiitem() {
	...
	if(present("orb",this_player())) {
		write("You already have your guilditem.\n");
		return 1;
	}
	write("You got your guilditem.\n");
	move_object(clone_object("/wizards/astarel/psi/psiorb"),this_player());
	return 1;
}
...
status psihelp() {
	...
	write(read_file("/wizards/astarel/psi/psihelp"));
	return 1;
}
