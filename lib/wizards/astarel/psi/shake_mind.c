...
resolve_skill(str) {
    ...
    if(target->query_level() > SM_LVL_LIMIT) {
    ...
    if(this_player()->query_sp() < SM_SP_NEEDED) {
    	
    	write("You lack the mental strength to accomplish this attack.\n");
    	return 1;
}
this_player()->reduce_sp(SM_SP_NEEDED);
...
fail_skill(str) {
    ...
    write("You to concentrate on shaking " + target->query_name() + "'s mind, but fail.\n");
