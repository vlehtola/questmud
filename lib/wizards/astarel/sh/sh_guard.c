...
void guard_reset() {
  ...
  set_random_pick(0);
  temp = this_object()->query_max_hp();
  if(guard_name)set_short(capitalize(guard_name) +" the guard.");
  temp /= 10;
  set_regen(temp);
  set_str(this_object()->query_str()*4);
  set_dex(this_object()->query_dex()*4);
  set_int(this_object()->query_int()*4);
  set_wis(this_object()->query_wis()*4);
  set_skill("dodge",100);
  set_skill("doublehit",100);
  set_skill("cast essence",200);
  set_skill("cast fire",200);
  set_skill("mana control",200);
  set_skill("cast bolt",200);
  set_skill("cast storm",200);
  set_skill("cast blast",200);
