...
update_stats() {
    object ob;
    int i, stat, scar_modifier, hpr, t;
    string c, *eq_res;
...
add_skill_exps(int num,setti) {
	...
	if (skill_exps[i] >= skill_costs[i] r skills.\n");
		}       
	}
	i += 1;
    }
...
{ 
 if(strlen(str) > 500) 
   write("Too long string. Aborting.\n");
	else
   desc=str; 
}
...
long() {  
	...
	switch(query_alignment()) 
	 {
	  case 50..300 : 
		...
		break;
	  case -4		t = "an extremely good"; break;
		case 50..99:
        			t = "a good"; 		break;
		case 10..49:
	...
	if(desc)
       {
        string buffy,desc_temp;
	desc_temp=desc;

         while(strlen(desc_temp))
         {
          sscanf(desc_temp,"%s\n%s",buffy,desc_temp);
          write(">"+buffy+"\n");
         }
...
kill(string str) {
    ...
    if (ob == this_object()) {
	write("What? Attnger()*6 < query_max_hunger()) {
...
fix_weight() {
  object ob;
  ob = first_inventory(myself || query_level() < 5) {
...
for(i=0;i<sizeof(groups);i++) {
   if(!news_data && i < (sizeof(groups)-1)) {
    ...
    if (!ghost && !guest) {
      call_other(find_object("/daemons/uest_points();
...
int channels(string arg) {
  ...
  if(channcomm=="topic") {
    ...
    CHANNEL_D->change_topic(channame,\n");
      return 1;
    }
