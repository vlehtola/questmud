...
"Hobbit says: If you ever need any herbs or something, just ask me.\n";
    ...
    load_chat(40, chat_str); /* load echos when not in combat with 40% propability*/
    load_a_chat(100, a_chat_str); /* load echos used when  in fight */
    /* ..and a weapon */
...
monster_init() {
   ...
   /* str == any line echoed to monster */

catch_tell(str) {
   string tmp1,tmp2;
   ::catch_tell(str);
  /* check if someone gives a painting */
   if (sscanf(str, "%s gives painting %s.", tmp1, tmp2) == 2) { 
	if (present("painting", this_object()) && this_player()->query_name() == tmp1) {
	   ...
	   /* quest functions. quest is complete */
	   if (!this_player()->query_quests("hobbit gardener")) {
	      this_player()->set_quest("hobbit gardener");
	      this_player()->add_exp(200000);
	   }
	}
   }
}
