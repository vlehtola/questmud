...
death(arg) {
  ...
  say("Mummy's "+mummy_limbs[limb_count]+" is cut off, but otherwise it seems unharmed!\n");
  limb_count += 1;
...
reset(arg) {
  ...
  org_long = long_desc;
  mummy_limbs = ({"left leg", "right leg", "left arm", "right arm", "head", });
  set_mage(1);
  set_undead();
  set_aggressive(1);
// no block to rebirthroom. ++C
