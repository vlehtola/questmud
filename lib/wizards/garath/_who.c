...
cmd_who(string str) {
  ...
  if(options) {
    if(member_array("short",options) != -1) {
      result = "";
      numba = 0;
      resulti = "QuestMud - http://www.laeppae.com/~quest - telnet://quest.laeppae.com:3000 \n";
      for(a=0;a<size;a++) {
	   ...
	   result += "[ Re ]";
           ...
           result += sprintf("[ %3d]",user[a]->query_level());
         result += sprintf(" %-9s",user[a]->query_name());
         resulti += result;
         numba++;
         if(a != size && numba >= 5) { resulti += "\n"; numba = 0; }
         else if(a != size && numba < 5) resulti += "";
         else resulti += "\n";
         result = "";
      }
      resulti += "\n"+size +" player(s) shown. \n";
