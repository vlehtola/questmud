...
#define SAVE_FILE_REASON "/data/banish_reason"

mapping names, sites, reasons;
int level, Wiza;
...
void reset() {
    ...
    if(file_size(SAVE_FILE_REASON + ".o") > 0) {
      restore_object(SAVE_FILE_REASON);
    }
}

mapping query_ban_name_time(string name) { return names[name,0]; }
mapping query_ban_site_time(string name) { return sites[name,0]; }

mapping query_name_banner(string name) { return names[name,1]; }
mapping query_site_banner(string name) { return sites[name,1]; }

string query_reason(string name) { return names[name, 3]; }
string query_sreason(string name) { return sites[name, 2]; }

string query_arch_reason(string name) { return reasons[name, 0];}

mapping query_site_level(string name) { return sites[name,3]; }

int add_banish_name(string str, int atime, string reason, string reason2) {
  object ob;
  object ob2;
  string tmp;
  int endtime;
  ...
  if(str && sscanf(str,"%s %s",str,tmp) != 2) {
    ...
    if(this_player()->query_level() <= level && Wiza) {
...
status info_name(string str) {
  ...
  write("ArchReason  : " + query_arch_reason(str) + ".\n");
