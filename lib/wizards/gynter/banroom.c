#include "room.h"
#define BANISH_D "/wizards/gynter/banish_d"
...
status add_name_2(string reason, int time, string who) {
  ...
  write("Enter Reason (Enter to skip) > ");
  input_to("add_name_reason", 0, reason, time, who);
  return 1;
}

status add_name_reason(string reason2, string reason, int time, string who) {
  if(!reason2) reason2 = reason;
  BANISH_D -> add_banish_name(who, time, reason, reason2);
  return 1;
}
