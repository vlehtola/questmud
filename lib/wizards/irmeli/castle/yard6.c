...
reset(arg) {
  if(!present("citizen")) { 
    move_object(clone_object("/wizards/irmeli/castle/monsters/citizen.c"),
    this_object());

}  
if(arg) return;

add_exit("north","/wizards/irmeli/castle/yard9.c");
add_exit("east","/wizards/irmeli/castle/yard2.c");
add_exit("northeast","/wizards/irmeli/castle/yard5.c");
...
long_desc = "A beautiful green grass lawn has conquered the castle yard and even the castle\n"+
            ...
            "covered with old blood. The platform is located towards northeast. There are two\n"+
