...
// 23.01.2003 no limbs are to be removed from the horde at ANY case ++Celtron
...
start_hpboost(int dur) {
        hpboost = 1;
        update_horde();
        call_out("remove_hpboost",dur+80);
}
...
start_statboost(int dur) {
        statboost = 1;
        update_horde();
        call_out("remove_statboost",dur+80);
}
start_specialboost(int dur,int effect) {
        specialboost = 1;
        specboost = effect;
        call_out("remove_specialboost",dur+80);
        update_horde();
}
remove_specialboost() {
        specialboost = 0;
	specboost = 0;
        tell_object(find_player(master),"The unnatural blue glow surrounding your horde disappears.\n");
        update_horde();
}

remove_hpboost() {
        tell_object(find_player(master),"The bright red aura surrounding your horde slowly vanishes.\n");
        hpboost = 0;
        update_horde();
}
remove_statboost() {
        tell_object(find_player(master),"The bright brown aura surrounding your horde slowly vanishes.\n");
        statboost = 0;
        update_horde();
}
...
command(string str) {
  ...
  if(str == "follow") {
    ...
    obs = filter(obs,"viltteri"); //changed from filter_array to filter
  if(obs) {
...
if(str == "fix") 
{
 if( time() - prev_fix < 10 ) // Nalle 16.3.2004
  {
   log_file("HORDE_FIXES", ctime(time())+ " "+ this_player()->query_real_name() + " ("+master+") fixed horde within 10s timeframe.\n",1);
   write("Horde fixed!\n");
   return 1;
  }
...
// actually destroy the spell, so it cannot resolve. fixed by Celtron 09.04.2004
 if(this_object()->query_spell()) destruct(this_object()->query_spell());
 return 1;
}
...
query_control_cost(string str) {
  int cost;
  if(str == "move") cost = total_undeads()/3;
  if(str == "attack") cost = total_undeads()*5;
  cost -= cost * MASTERY_D->query_mastery("Mastery of dark arts",find_player(master))/200;
  return cost;
}
...
extra() {
 ...
 enemy->hit_with_spell( (dk_special/2)+(specboost/2),"physical");
...
int i;
  ...
  if(obs) {
    for(i=0;i<sizeof(obs);i++) {
        if(obs[i] != this_object()) {
                if(obs[i]->query_horde_master() == master) {
                        write("Both hordes goes insane and attacks against each other!\n");
                        destruct(obs[i]);
                        destruct(this_object());
                }
        }
        ...
        if(follow) {
        if(find_player(master) && interactive(find_player(master))) {
                if(attacker_ob && present(attacker_ob, environment(this_object()) )) {
      if(undeads["skeleton"]) {
        write("The horde is in combat and the skeletons refuse to leave!\n");
        return 1;
...
+ 10;
  ...
  if(undeads["lich"] && !mage) {
    set_mage(1,(c_grea+c_magi-160)/2);
    set_invisible(0); // C
// (bugi?) seuraavaan jako kahdella by Celtron. 29.04.2004
    set_lich_skills( (c_magi+c_grea)/2 );
  }
  set_size(65);
  set_name("horde");
  tmp = get_horde_adj();
...
update_horde_hp() {
        ...
        ob = find_player(master);
        if(!ob) return;
        h_max_hp = (undeads["skeleton"]*4) + (undeads["wight"]*8) + (undeads["ghost"]*90) + (undeads["zombie"]*90) + (undeads["flesh golem"]*800) + (undeads["death knight"]*450);
        h_max_hp += ob->query_skills("control corporeal")+ob->query_skills("control ethereal")+ob->query_skills("control greater undead")+ob->query_skills("control dark magic")*9;
        h_level = undeads["skeleton"]+undeads["wight"]+undeads["ghost"]+undeads["zombie"]+undeads["flesh golem"]+undeads["wraith"]+undeads["death knight"]+undeads["flesh golem"]+undeads["lich"];
        h_level = h_level*2/3;
        i = this_object()->query_hp();
        set_level(h_level);
    if(h_max_hp > 3000) h_max_hp = 3000;
    if(query_hpboost() == 1) {
                h_max_hp += 1000;
        }
	h_max_hp += MASTERY_D->query_mastery("Mastery of dark arts",find_player(master))*5;
        set_max_hp(h_max_hp);
...
h_str = undeads["skeleton"] + undeads["wight"]*3 + (undeads["ghost"]*2) + undeads["death knight"]*18 + undeads["flesh golem"]*7;
h_dex = undeads["skeleton"] + undeads["ghost"]*10 + undeads["death knight"]*10;
h_int = undeads["skeleton"] + undeads["lich"]*90 + undeads["death knight"]*14;
h_wis = undeads["skeleton"] + undeads["lich"]*90 + undeads["death knight"]*14;
...
h_askill = undeads["skeleton"]/2 + undeads["wight"]*4 + undeads["ghost"]*2;
h_dskill = undeads["skeleton"]/2 + undeads["zombie"]*4 + undeads["flesh golem"]*10;
...
mastery_stat = MASTERY_D->query_mastery("Mastery of dark arts",find_player(master))*2/3;
h_askill += MASTERY_D->query_mastery("Mastery of dark arts",find_player(master))/10;
h_dskill += MASTERY_D->query_mastery("Mastery of dark arts",find_player(master))/10;
