...
duplicate(str) {
  object ob, ob2, target;
  int cost;
  string file, tmp, tgt;
  cost = 1000;
if (sscanf(str,"%s for %s",ob,tgt) != 2) { write("Syntax: duplicate key [number] for [name]\n"); return 1; }
...
ob = present(str, this_player());
  ...
  if(!target) { write(tgt+"is not present.\n");
  return 1;
  }
  this_player()->reduce_money(cost,3);
  write("You give your key to the officer and after a while he hands you\n"+
        "back the original and a duplicate of it.\n");

  file = file_name(ob);
  sscanf(file, "%s#%s", file,tmp);
  ob2 = clone_object(file);
  move_object(ob2, target);
  return 1;
}
