...
resolve_skill(target) {
    ...
    while(i<sizeof(ob)) {
    if(ob[i] != this_player() && living(ob[i]) && !living(ob[i])->query_animal_shadow()) {
if(!this_player()->query_party() || this_player()->query_party() != ob[i]->query_party()) {
...
fail_skill(target) {
    ...
    say(capitalize(this_player()->query_name()) + " tries to sleep, but wakes up with angry look on " + this_player()->query_possessive() + " face.\n", this_player());
}
...
awake() {
  ...
  hp_amount = this_player()->query_skills("camping")*3+this_player()->query_con()*3/2;
  ep_amount = hp_amount / 5;

  this_player()->add_hp(hp_amount);
  this_player()->add_ep(ep_amount);
/*
  this_player()->set_hp( ( this_player()->query_hp()*3 + this_player()->query_max_hp() ) / 4);
