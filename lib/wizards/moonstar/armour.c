...
start() {
  ...
  set_long("The scale mail seems to be blessed by divine force.\n"+
        "It looks unique in it's beauty. The strong scales of unknown\n"+
        "material fit perfectly in their places but still they give up so much\n"+
        "that the wearer is almost not strained at all.\n"+
        "The insignia of power is craftly carved on the chest covering part.\n"+
        "Pair of runes are carved on the scales that cover the right shoulder.\n");
...
boost_all() {
  write("HOP HOP MITÄ TAPAHTUU!?!?!!11.\n");
        clone_object("/wizards/moonstar/test/bless_shadow.c")->start(this_player());
  worn_by->update_stats();
  return 0;
}
