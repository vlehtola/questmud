...
start(ob) {
  ...
  con -= con/10;
  dex = mina->query_skills("power of thunder")+i;
...
query_focus_elements(string type, int dam) {
  if(!type) return 1; /* pelkkä onko focusia vai ei */
  if(type == "hit") {
    /* [10,30] */
    return (str/10)+10;
  }
  if(type == "damage") {
    return (dam*str / 150)+5;
  }
}
...
end_focus_elements() {
  ...
  tell_room(environment(mina), mina->query_name()+" powers leaves.\n", puuh);
  str=0; con=0; dex=0;
  mina->update_stats();
  mina->add_hp(1);
