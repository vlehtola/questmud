...
resolve_skill(str, object ob) {
  ...
  dama = (this_player()->query_skills("sky strike")*2/3+this_player()->query_skills("claws storm")/2 + this_player()->query_skills("dragon blow")*dama/200 + dama/4*3 + this_player()->query_skills("body anatomy")*6/3);
    if(ob->query_wiz()) tell_object(ob,"Skill: "+weapon_skill+" Dam: "+dama+"\n");
if(dama >1800) { dama = 1800;  }
