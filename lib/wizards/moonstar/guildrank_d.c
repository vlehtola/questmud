...
get_rank_status(string guild, int i) 
{
  ...
  if(guild == "fighter")
    ranks = ({"a brawler", "a soldier", "a corporal", "a sergeant", "a lieutenant", "a commander", "a general","a Champion",});
  if(guild == "cleric")
  ...
  if(guild == "martial artist")
    ranks = ({"a takitu", "a ryotaka", "a zasenro", "a wahringust", "a sabumnim","a sarinku", "a saizenso", "a saizenso", "a Master of mind and body",});
  if(guild == "sorcerer")
...
query_rank(mixed str, string guild, status verbose) {
  ...
  if(guild && is_member_of_guild(ob, guild)) {
    ...
    if ( get_rank_level(guild, rank_score ) == 7 ) {
      return best_phrase(ob, rank_status);
    }
  ...
  while(i < sizeof(guild)) {
    if(is_member_of_guild(ob, guild[i])) {
      ...
      if ( get_rank_level(guild[i], rank_score ) == 7 ) {
        rank_status += best_phrase(ob, get_rank_status(guild[i], rank_score ) );
      } else {
...
string best_phrase(object ob, string rank_status) {
  if ( !rank_status ) return "";
