// Korjasin hitaan tulostuksen ja oudon lopun colorbuffer muuttujalla ja
// poistamalla toistuvat ansikoodit, jos edellinen on sama.
// -N 5/04
...
"  W +--+--+--8--+--+--+--+          W   Lb: Library          Tl:Teleport room\n"+
"  WWWWWWW | Au  Ar |     |          W   Mg: Mages guild\n"+
"        W +      Ab+     +          W   Me: Merchants guild\n"+
...
{
 int i;
 string color, colorbuffer, prevcolor;
 ...
 if(this_player()->query_terminal())
 {
  for(i=0;i<strlen(str);i++)
  {
   ...
   if(str[i..i]=="+") { color = YELLOW_F; }
   if(str[i..i]=="-") { color = YELLOW_F; }
   if(str[i..i]=="|") { color = YELLOW_F; }
   if(str[i..i]=="=") { color = CYAN_F;   }
   if(str[i..i]=="*") { color = BOLD;     }

   if(prevcolor != color)
     {
      colorbuffer += OFF + color + str[i..i];
     }
      else
     {
      colorbuffer += str[i..i];
 ...
 //Eli jos ei oo v^Drit p^D^Dll^D ni pistet^D^Dn pelkk^D str
 else
  {
  write(str);
 }
...
drawmap(int x, int y)
{
 ...
 write("\n");
 sscanf(map_a,"%s\n%s",arr[i],temp);
 for(i=0;i<27;i++) if(!sscanf(temp,"%s\n%s",arr[i],temp)) break;
 y *= -1;
  for(i=0;i<27;i++)
    {
     if(!arr[i]) break;
