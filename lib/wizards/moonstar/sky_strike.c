...
dama = dama + this_player()->query_dex();
  ...
  case 2101..2700:
    ...
    break;
  case 2701..3500:
    tell_object(ob, "You close your eyes and focus your mind and body for ultimate strike while screaming "+RED_F+"'OMNISHSLASH'"+OFF+"\n");
    tell_object(ob, "A blinding red light gomes out from your knuckles and hits "+target->query_name()+" while you jump high in the air.\n");
    tell_object(ob, "Flying through the air you STRIKE  "+target->query_name()+"'s face and TEAR "+target->query_objective()+" body with FORCE!\n");
    tell_object(target, ob->query_name()+" flies towards you and STRIKE your face and RIPS your skin off with FORCE!\n");
    tell_room(environment(ob), ob->query_name()+" focuses all "+ob->query_objective()+" power and screams "+RED_F+"'OMNISHSLASH'"+OFF+".\n", ({ ob, target}));
