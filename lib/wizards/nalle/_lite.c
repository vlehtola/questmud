...
{ write("You have no colors set, default colors are used.\n"); return 1; }
  ...
  switch(temp[0])
  {
    case "channel":
    {
        if(!temp[2]) { write("You must specify a color.\n"); return 1; }

        if(!channel_exists(temp[1])) 
...
switch(str)     {
        ...
        case "green": return RED_F;
        case "light_green": return BOLD+RED_F;
...
for(i=0;i<x;i++)        {
        if(arr[i]==str) { return return_color(l[str,0]); }
                        }
return "";
