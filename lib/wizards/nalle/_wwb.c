...
string *buffy;
  ...
  for(i=0;i<sizeof(chan_members);i++) 
   ...
   if(ob) tell_object(ob,construct_chanstr(ob, str));
   }
...
for(i=0;i<sizeof(chan_members);i++) 
 if(find_player(chan_members[i])) 	{
   ...
   result+=capitalize(chan_members[i])+" idle "+idlestring+", ";
						}
tell_object(TP,result[0..(strlen(result)-2)+"\n");
return 1;
}
