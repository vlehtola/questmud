...
reset(arg) {
    ...
    set_long("This is one of the Dequan Knights that patrol the streets of Chamron.\n" +
             "He seems like a skilled warrior, and a worthy opponent.\n");
    set_al(10);
    set_aggressive(0);
    set_move_at_random(1 + random(2));
        if (!chat_str) {
        chat_str = allocate(3);
        chat_str[0] =
