...
#define NAME_OF_GHOST                   "ghost of "+name
...
move_player(string dir_dest, object optional_dest_ob) {
  ...
  if(query_attacker() && environment(query_attacker()) == environment(this_object()) &&
	dir != "X" && dir != "XX" && fleeing != "flee" && !query_npc()) {
    if(fleeing) { 
  ...
  if(is_light && !this_object()->query_invisible()) {
    if (is_light < 2) { temp_name = "Something"; } else { temp_name = query_name(); }
    if (!msgout) msgout = "leaves";
    if (dir == "X" || dir == "XX")
      say(temp_name + " " + mmsgout + ".\n");
    else
      say(temp_name + " " + msgout + " " + dir + ".\n");
  }
  ...
  if(is_light && !this_object()->query_invisible()) {
    if (is_light <= 1) { temp_name = "Something"; } else { temp_name= query_name(); }
    if (!msgin) msgin = "arrives";
    if (dir == "X" || dir == "XX")
      say(temp_name + " " + mmsgin + ".\n");
    else if (arrive_dir)
      say(temp_name + " " + msgin + " from the " + arrive_dir + ".\n");
    else 
      say(temp_name + " " + msgin + ".\n");
  }
  ...
  if(ob && ob->query_leader() == this_object()) {
...
if(environment(this_object())->query_property("no_kill")) return 0;
  ...
  hit_point = hit_point - dam;
  if(ob) ob->add_party_exp(random(dam)*10);
  if (hit_point < 1) {
    death = 1;
    dead_by_hit = dam_type;
    this_object()->death();
...
death() { 
  ...
  if(query_npc() && attacker_ob) {
    int tune;
    tune = experience;
    if(tune > 2000000 && !attacker_ob->query_wiz())
  ...
  // returns a percent from 0 to 100, fixed by Nalle 01Aug02  
  if(random(100)<this_object()->query_rebirth("avoid death")) {
    dead = 0;
  ...
  if (!this_object()->second_life() && query_npc()) {
    ...
    if(!this_object()->query_no_corpse()) {
    ...
    "/daemons/channel_d"->send_to_channel("inform", query_real_name()+" dies.", this_object());
  }
  return 1;
}
...
get_weapon_skill(name_of_left_weapon) {
  ...
  if(!name_of_left_weapon) return this_object()->query_skills("bare hands");
	...
	if (name_of_left_weapon->query_type() == 5) {
	   wepa = this_object()->query_skills("polearms");
	}
...
restore_time_points() {
  ...
  // PSI Combat
  if(this_object()->query_skills("mental strike")) {
  ...
  if(this_object()->query_spell() && !this_object()->query_npc()) {
    tell_object(this_object(), "You are unable to fight while casting a spell.\n");
    return 1;
  }
...
casting_sp_reduce(int num) {
	...
	write("You are beginning to feel exhausted.\n");
    } else if (ep_loss && endurance_points > max_ep / 2) {
	write("You feel exhausted from the spellcasting.\n");
    } else if (ep_loss && endurance_points > max_ep / 3) {
	write("You feel VERY exhausted from the spellcasting.\n");
    } 
    if (endurance_points < 0 && !is_npc) {
	write("Everything goes dark and you drop unconcious from exhaustion.\n");
        if (spell) {
