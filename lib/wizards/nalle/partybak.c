...
/* member_exp ([ object player : int exp_gained ]) */
mapping members, member_exp;
string party_name, *channel_last;
int last_message;
object leader_ob; 	// who leads the party

static int total_exp, creation_time;
static object invite_flag;
static mapping kills, party_follow;
...
party_commands(string str) {
  object party, invite_ob, leader_o, ob;
  string p_name;
  int x,y;
  ...
  if( str == "shake") {
    ...
    party_say("shakes the party. All members stop following.", this_player()->query_name(), 1);
  ...
  if (sscanf(str, "name %s", str) == 1) {
    if(this_player()->query_party()->query_leader() != this_player()) {
       write("You are not the leader.\n");
       return 1;
    }
    write("You rename your party to '"+str+"'.\n");
    this_player()->query_party()->set_name(str);
    return 1;
  }
...
void show_party_messages() {
  int i;
  for(i=last_message; i >= 0; i--) {
    if(channel_last[i])
	write(channel_last[i]+"\n");
  }
  for(i= sizeof(channel_last)-1; i > last_message; i--) {    
    if(channel_last[i])
	write(channel_last[i]+"\n");
  }
}
...
void change_leader_to_next() {
  ...
  // no active members
  write("No more active members, party destroyed.\n");
  destruct(this_object());
}
...
status put_in_place(object player, int x, int y) {
  if(member_in_place(x,y)) return 0;
  members += ([ player : ({x,y,}) ]);  
  return 1;
}
