...
e_map=([
  ...
  "argh":"go 'Argh!'";"goes 'Argh!'";"at";0;0;0;0;0,
  "bark":"bark";"barks";"at";0;0;0;0;0,
  ...
  "click":"click";"clicks";"at";0;0;0;0;0,
  ...
  "huggle":"huggle";"huggles";0;"bear-likely";0;0;0;1,
...
// A pseudo target thing and set the target object for says
if(target) if(!find_player(target)) pseudo_target=1; else
target_ob=present(target,environment(this_player()));
...
if(!target && e_map[verb,5]) { target=e_map[verb,5]; pseudo_target=1; }

//If the target is an object -> pseudo_target and no bodypart
if(target) if(!living(target_ob)) { pseudo_target=1; bodypart=0; }
...
// And the pseudo_target (from mapping) is much more handy without preps
if(e_map[verb,2]) { prep=" "+e_map[verb,2]+" "; } 
else { prep=" "; }
if(e_map[verb,5]) prep=" "; // this is because of objects
...
if(!e_map[verb,6]) 
{ bprep=" "; if(at_self) target=this_player()->query_possessive(); } 
