...
resolve_skill(str) {
    ...
    if(dama < 10) {
      write("You kick " + capitalize(target->query_name()) + " in the
knee!\n");
    ...
    if(dama > 20) { dama=20; }
    if(this_player()->query_attack() != target)
this_player()->attack_object(target);
    if(!random(1000)) {
     this_player()->set_mastery("Offensive
maneuvers",(this_player()->query_mastery()["Offensive maneuvers"] + 1));
   }
...
fail_skill(str) {
    ...
    tell_object(target, this_player()->query_name() + " tries to KICK you, but
misses.\n");
    say(capitalize(this_player()->query_name()) + " tries to kick " +
target->query_name() +
       ", but misses.\n", target);
