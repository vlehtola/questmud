...
reset(arg) {
  ...
  long_desc = "You are standing in a great library.\n";
...
random_books() {
   ...
   while(i<sizeof(books)) {
	books[i] = random(call_other("/wizards/onarion/library_fun","number_of_books",1))+1;
	long_desc += "   "+call_other("/wizards/onarion/library_fun","get_short",books[i])+"\n";
	i += 1;
   }
}
