...
join() {
        if(!joins[TP->query_name()]) {
                ...
                joins += ([TP->query_name(): 1]);
        } else {
                tell_object(TP,"Hoxbah says: 'You have already joined the event!\n'");
        }
        if(!present("map",TP)) {
                move_object(clone_object("/wizards/rag/events/thunt/map"),TP);
...
add_score(object ob) {
        ...
        if(score[name] == WIN) {
                ...
                prize = (random(6)+5)*10;
                pl->add_money(prize,3);
                tell_object(pl,"Hoxbah tells you 'Good job! You receive "+prize+" bronze.'\n");
