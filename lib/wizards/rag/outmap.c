...
object room_loaded(object room, int x, int y, string map_symbol) {
  ...
  //If the whole party moves, only the leader can get ambushed -- Rag 2.7.2004
  if( this_player()->query_party() && !present("party object", this_player()) &&
  		environment(this_player()->query_party()->query_leader()) == environment(this_player()) ) {
  	return 0;
  }
    
  //Added by Rag 25.5.2004
  if(this_player()->query_rebirth() > 2) {
  	tell_object(this_player(), "Your knowledge of the world allows you to avoid an ambush.\n");
  	return 0;
  }
