...
reset(arg) {
 ...
 set_light(3);
 set_not_out(1);
 short_desc = "Red control room";
 long_desc = "In this room player controls his/her pet.\n See 'do_attack', 'do_defence' and 'clear'.\nType 'start' when you are ready. The battle begins when both, red and blue,\nare ready.\n";
 add_exit("southeast", "/wizards/rimber/test/colosseum/room1.c");
}
...
clear(string str) {
 if(!str) {
  write("Commands:\n 'clear all' - Clears all attacks and defences.\n 'clear attack' - Clears all attacks.\n 'clear defence' - Clears all defences.\n");
 }
 if(str == "all") { a = 0, d = 0; }
 if(str == "attack") { a = 0; }
 if(str == "defence") { d = 0; }
...
start() {
 ...
 r = 1;
 write("You are ready.\n");
 red_player = this_player()->query_name();
 "/wizards/rimber/test/colosseum/arena.c"->start();
 return 1;
}

string red_player() { return red_player; }
