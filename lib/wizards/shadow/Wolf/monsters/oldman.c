...
catch_tell(str) {
    ...
    if (sscanf(str, "%s gives %s to %s", giver, item, tmp) == 3) {
      ...
      if (!ob) { return; }
      item_ob = present("mc_quest_ring", this_object());
      if (!item_ob) { return; }
//Fixed by Rag, eq-short's don't work right because of Nalle :)
/*
...
*/
      if (ob && item_ob && !ob->query_quests("Return Layla's ring to the old man")) {
