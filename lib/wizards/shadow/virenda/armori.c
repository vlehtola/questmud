...
TWO_EXIT("players/shadow/virenda/road2", "east",
	"players/shadow/virenda/storage1", "west",
	 "The shop",
"You are in an armour shop. You can buy or sell things here.\n" +
...
do_sell(ob) {
    int value, weapon, armour, do_destroy;
    value = ob->query_value();
    if (!value) {
	write(ob->short() + " has no value here. Try another shop.\n");
	return 1;
    }
    weapon = ob->query_value();
    if (weapon) {
	write(ob->short() + " has no value here. Try another shop.\n");
	
	return 1;
    }
    armour = ob->query_value();
    if (armour) {
	write(ob->short() + " has no value here. Try another shop.\n");
    ...
    call_other("room/store1", "store", ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
	if (call_other("room/store1", "value", item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
    call_other("room/store1", "buy", item);
    return 1;
}
...
north() {
    ...
    call_other(this_player(), "move_player", "north#room/store1");
    return 1;
}

list(obj) {
    call_other("room/store1", "inventory", obj);
    return 1;
}
