...
TWO_EXIT("players/shadow/virenda/road2", "west",
	"players/shadow/virenda/storage2", "east",
	 "The shop",
"You are in a weapon shop. You can buy or sell things here.\n" +
...
do_sell(ob) {
    int value, shield, jacket, robe, skirt, do_destroy;
    ...
    call_other("room/store2", "store", ob);
    return 1;
}
...
value(item) {
    ...
    if (!name_of_item) {
	if (call_other("room/store2", "value", item))
	    return 1;
	write("No such item ("); write(item); write(") here\n");
...
buy(item) {
    if (!item)
	return 0;
    call_other("room/store2", "buy", item);
    return 1;
}
...
north() {
    ...
    call_other(this_player(), "move_player", "north#room/store2");
    return 1;
}

list(obj) {
    call_other("room/store2", "inventory", obj);
    return 1;
}
