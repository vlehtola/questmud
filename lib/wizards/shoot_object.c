...
start_shoot(w, t) {
  ...
  if(!with->query_reload()) { write("You can't shoot with the '"+w+"' because its not reloaded!\n"); stop_shoot(); return 1; }
  p_ob = this_player()->query_party();
  if(p_ob && !p_ob->first_in_row(this_player())) {
  write("You can't shoot anything from back of the party.\n");
  stop_shoot();
  return 1;
}
...
switch(hit_chance) {
	case 0..75:
	write_lousy_messages(damage);
	break;
	case 76..150:
	write_mediocre_messages(damage);
	break;
	case 151..200:
	write_good_messages(damage);
	break;
	case 201..500:
	write_crits_messages(damage);
	break;
}
if(random(150) < this_player()->query_skills("puncturing shots")) {
...
write_mediocre_messages(int dam) {
switch(dam) {
case 0..100:
