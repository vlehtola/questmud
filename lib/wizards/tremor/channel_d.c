...
print_log(string file_name, string str) {
  if(!this_player() || !this_player()->query_name()) return 0;
  if(str == 0 || str == "") return 0;
  if(sscanf(str, "%s\n", str) == 1) { }
  this_object()->send_to_channel("log","*log add on file '"+file_name+"', "+str+"*");
}
...
reset(arg) {
  ...
  channels=([ "inform": 5; "ChannelD"; "Gameplay info";           ({ }), 
              ...
              "code": 1; "ChannelD"; "Inside Discussion about code [*MoronShield*]";            ({ }),
              "wanted": 0; "ChannelD"; "Ask for stuff you want";            ({ }),
              "alert": 0; "ChannelD"; "Warn others about area damaging";            ({ }),
...
send_to_channel(chan,msg, object this_obj) {
  ...
  if(member_array(chan,temp)!=-1 || channels[chan,0]==5) {
	...
	if(!who[i]->query_ignore(lower_case(this_obj->query_real_name()))) {
