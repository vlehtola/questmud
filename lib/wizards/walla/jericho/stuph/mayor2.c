...
catch_tell(str) {
    ...
    if (sscanf(str, "%s gives %s to %s", giver, item, tmp) == 3) {
      ...
      if (!item_ob) { return; }
      if (ob && item_ob && item_ob->query_name() == "necklace" ) {
...
"Aloc Acoc says: 'Here, you can have this necklace back. I tuned\n"+
        "it a bit, to give you more powers. I hope you don't mind'.\n");
