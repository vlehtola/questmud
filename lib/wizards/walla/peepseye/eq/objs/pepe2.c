...
catch_tell(str) {
    ...
    if (sscanf(str, "%s gives %s to %s", giver, item, tmp) == 3) {
      ...
      if (item_ob->short() != "A glasseye, made of porcelaine") {
         tell_object(ob, "Pepe mutters: 'I don't want your bloody "+
                "rubbish.'\n");
         tell_object(ob, "Pepe chucks your "+capitalize(item)+
                " back at you.\n");
         move_object(present(item,this_object()), ob);
         return;
      }
      if (ob && item_ob && item_ob->short() == "A glasseye, made of porcelaine" &&
                !ob->query_quests("Layla")) {
         tell_object(ob, "You fullfill the quest and blahblah blha you must be proud of yourself now.\n");
