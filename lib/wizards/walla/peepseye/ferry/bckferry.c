...
reset(arg) {
   ...
   long_desc = "You are on a ferry of some sort. It is not big, and it seems that if the wind amplifies\n"+
                 ...
                 "It seems that there is noone steering the whole thing, but it is being pulled from\n" +
                 "somewhere.\n";
   set_not_out(1);
...
start_ferry() {
   location = 1;
   add_exit("out", "wizards/walla/peepseye/kitty/dock.c");
...
real_move() {
 ...
 "The ferry arrives.\n");
        ...
        call_other("wizards/walla/peepseye/kitty/dock.c", "add_exit",
                "north","wizards/walla/peepseye/ferry/ferry.c");
        add_exit("out", "wizards/walla/peepseye/1.c");
        location = 2;
        call_out("change_pos", 30);
        return 1;
   }
