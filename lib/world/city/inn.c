// commented out the log_files. Celtron 20051102

inherit "/room/room";

/* 10min limit */
status check_if_idle(object user) {
//  log_file("INN", ctime(time())+" "+ user->query_name() +" is idle?\n",1);
  if(living(user) && query_idle(user) > 600) return 1;
  return 0;
}

status move_to_tavern(object user) {
  user->move_player("east#/world/city/tavern");
//  log_file("INN", ctime(time())+" "+ user->query_name() +" moved.\n",1);
  return 1;
}
...
//  log_file("INN", "arraysize is "+sizeof(idle_users)+".\n", 1);
  idle_users = filter_array(idle_users, "move_to_tavern");
...
safe_quit() {
    ...
    while(ob) {
        ...
        if (i > 25) {
            write("You have more than 25 items, cannot safe_quit.\n");
            return 1;
        }
